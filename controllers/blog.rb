...
class Blog < Ink::Controller
  ...
  def index
    ...
    @posts = Ink::Database.database.find "entry", "ORDER BY `created_at` DESC LIMIT #{@steps + 1} OFFSET #{(@page - 1) * @steps}"
    ...
    @posts.each do |post|
      post.user = Ink::Database.database.find_references "entry", post.pk, "user"
      post.comment = Ink::Database.database.find_references "entry", post.pk, "comment"
    end
  ...
  def entry
    ...
    @post = Ink::Database.database.find "entry", "WHERE #{Entry.primary_key[0]}=#{@id}"
    ...
    if @params[:post] and @params[:post]["submit"] == "Add"
      # if there was no session, the user had to specify username and password, that we
      # authenticate here
      user = (not @u) ? User.authenticate(@params[:post]["user"], @params[:post]["pass"]) : @u
      ...
      if user and user.can? "Comment"
        ...
        c = Comment.new "title" => @params[:post]["title"], "text" => @params[:post]["text"], "created_at" => Ink::Database.format_date(Time.now)
        c.entry = @post
        c.user = user
        c.save
      end
    ...
    @post.comment = Ink::Database.database.find_references "entry", @id, "comment", "ORDER BY comment.created_at ASC"
    @post.comment.each do |comm|
      comm.user = Ink::Database.database.find_references "comment", comm.pk, "user"
    end
    
    # rendering is similar to the one in the index module
    render :template => "index", :locals => [ render :partial => "blog/single" ]
  end
